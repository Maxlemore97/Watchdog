...
Watchdog gates AI-agent–mediated package installs and agent-extension
loading. It is host-agnostic and LLM-agnostic: the "agent" can be any
LLM-driven host that runs package managers or loads pluggable
extensions — Claude Code, Claude Desktop, Cursor, Continue, Zed,
Cline, OpenCode, Aider, or a plain shell driven by an agent. The
LLM that performs the analyzer review is equally a swap-in (Claude,
Gemini, OpenAI, Ollama, or a custom CLI via `WATCHDOG_LLM_CMD`); the
engine and the threat model below apply uniformly across all of them.
The trust boundary sits between **untrusted artifacts** (published
packages, git-cloned plugins, install commands generated by an LLM)
and the **host machine running the agent**. The implementation is a
static Go binary; the defenses below are Go (stdlib + `regexp/RE2`,
`archive/tar`, `os/exec`).

Watchdog ships four adapters that share one engine: `watchdog-shim`
(package-manager PATH wrappers, any host that shells out),
`watchdog-mcp` (any MCP-aware host), `watchdog-pretool` /
`-session` / `-prompt` (Claude Code hook entry points), and
`watchdog-action` (GitHub PRs). Names of specific host config files
or extension formats below are representative, not exhaustive — the
same threat-model entries apply to the analogous artifacts in any
other host.
...
- **LLM territory — natural-language attack surface no CVE database
  ...
  can't help:
  - Agent extensions across hosts — plugin, skill, command, rule,
    or agent-definition files in Markdown or JSON. Concrete shapes
    Watchdog has been verified against include
    `.claude-plugin/plugin.json`, Claude Code `skills/` and
    `commands/`, `agents.md`, `CLAUDE.md`, Cursor rules, Continue
    config, Zed context-server entries, Cline MCP settings, and
    generic MCP server manifests. The same checks apply to any
    other host's equivalent files: prompt injection, hidden
    instructions, exfil patterns, declared-vs.-actual-tool
    mismatch.
  - Hooks and agent-side scripts (shell embedded in JSON or
    Markdown — PreToolUse, on-install, on-load, or any host's
    equivalent) — intent check on the embedded commands.
  - Plugin / extension manifests — sketchy permissions, unexpected
    network calls, install-time persistence into any agent's
    config directory (`~/.claude/`, `~/.cursor/`, `~/.continue/`,
    `~/.config/zed/`, VS Code extension storage, or equivalent).
...
4. **LLM review of install hooks + agent artifacts**
   ...
   `no_install_hooks`). For agent extensions across hosts (plugin /
   skill / command / rule / agent files in Markdown or JSON, plus
   any host-specific hook scripts), the full text surface is
   reviewed since no CVE database covers prompt-injection or
   exfil-pattern text.
